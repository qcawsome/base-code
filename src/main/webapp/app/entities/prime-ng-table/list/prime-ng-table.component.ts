...
import { EntityArrayResponseType, PrimeNgTableService } from '../service/prime-ng-table.service';
...
export class PrimeNgTableComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPrimeNgTable): number => this.primeNgTableService.getPrimeNgTableIdentifier(item);
  ...
  delete(primeNgTable: IPrimeNgTable): void {
    const modalRef = this.modalService.open(PrimeNgTableDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.primeNgTable = primeNgTable;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.primeNgTables = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPrimeNgTable[] | null): IPrimeNgTable[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.primeNgTableService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
